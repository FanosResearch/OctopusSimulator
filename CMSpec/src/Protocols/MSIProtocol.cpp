                                                        ...
                                                        GenericCacheLine &cache_line, int next_state)
    {
        ...
        for (int action : actions)
        {
            ...
            case ActionId::GetM:
                ...
                ((Message *)controller_action.data)->to.push_back((uint16_t)this->m_shared_memory_id[AddrMapping::getAddrMapping()->get_bnk_bits(msg.addr,this->m_core_id)]);
                ((Message *)controller_action.data)->llc_addr_restore = msg.llc_addr_restore;
                break;
            case ActionId::PutM:
                // send Bus request, update cache line
                controller_action.type = ControllerAction::Type::SEND_BUS_MSG;
