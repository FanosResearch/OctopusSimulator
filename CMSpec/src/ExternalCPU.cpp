 ...
 */
 ...
 namespace ns3
 {
     ...
     void ExternalCPU::processLogic()
     {
         ...
         while (true)
         {
             ...
             if(ready_msg.source == Message::Source::LOWER_INTERCONNECT)
             {
                 ...
                 Logger::getLogger()->addRequest(this->m_id, ready_msg);
             }
             else if(ready_msg.source == Message::Source::UPPER_INTERCONNECT)
             {
                 if(m_cpu_callback != NULL)
                     (*m_cpu_callback)(ready_msg.addr, this->m_clk_cycle, 
                                     (RequestType)ready_msg.complementary_value, 
                                     ready_msg.data);
                 else
                 {
                     cout << "ExternalCPU(id = " << this->m_id << "): ";
     ...
     {
         //fprintf(stderr, "addRequest: %lu %d %p %d\n", address, type, data, size);
     ...
     void ExternalCPU::setLoggerEnable(bool enable)
         {
            Logger::getLogger()->setLogEnable(enable);
         }
