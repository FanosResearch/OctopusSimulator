...
namespace ns3
{
    ...
    MainMemoryController::MainMemoryController(MCoreSimProjectXml &projectXml, CommunicationInterface *lower_interface, vector<int> llc_id) : ClockedObj(projectXml.GetDRAMCtrlClkNanoSec())
    {
    ...
    void MainMemoryController::processLogic()
    {
        ...
        if (ready_msg.data == NULL) //Read message 
        {
            ...
            if (ready_msg.owner >= 50)
                msg.to.push_back((uint16_t)m_llc_id[ready_msg.owner-50]); 
            else
                msg.to.push_back((uint16_t)m_llc_id[AddrMapping::getAddrMapping()->get_bnk_bits(ready_msg.addr,ready_msg.owner)]);     // To
            msg.llc_addr_restore = ready_msg.llc_addr_restore;
