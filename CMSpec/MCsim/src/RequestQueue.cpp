...
unsigned int RequestQueue::getRequestorSize(bool requestor, unsigned int requestorID)
{
	if (requestor)
	{
		if (requestorOrder.size() == 0)
		...
		{
			// No such requestor
			if (requestorBuffer.find(requestorID) == requestorBuffer.end())
			{
				return 0;
			}
			else
			{
				return requestorBuffer[requestorID].size();
			}
		}
	}
	else
...
Request *RequestQueue::getRequestorRequest(unsigned int RequestorID, unsigned int index)
{
	// Scan the requestorqueue by index, instead of requestorID value
	if (requestorBuffer[RequestorID].empty())
	{
		return NULL;
	}
	else
	{
		scheduledRequest.first = true;
		scheduledRequest.second = std::make_pair(RequestorID, index);
		return requestorBuffer[RequestorID][index];
	}
}
...
void RequestQueue::removeRequest(unsigned int requestor, unsigned long long id)
{
	for (unsigned int x =0 ; x< requestorBuffer[requestor].size() ; x++)
	{
		if (requestorBuffer[requestor][x]->reqID == id)
		{
			requestorBuffer[requestor].erase(requestorBuffer[requestor].begin() + x);
			return;
		}
	}
	cout << "Error RequestQueue: cannot find the removed request: "<< id << " core: " << requestor<< endl;
	return;

}
void RequestQueue::resetScheduledFlag (unsigned int requestor, unsigned long long id)
{
	for (unsigned int x =0 ; x< requestorBuffer[requestor].size() ; x++)
	{
		if (requestorBuffer[requestor][x]->reqID == id)
		{
			requestorBuffer[requestor][x]->scheduled = false;
			return;
		}
	}
	cout << "Error RequestQueue: cannot resetScheduledFlag request: "<< id << " core: " << requestor<< endl;
	return;
}
