...
MCsim::Request *MemoryController::hitRequest = NULL;
...

{
	...
	stats.closeWrite_Latency = 0;
	memoryDevice = NULL;
	parentMemorySystem->requestorsQueues = &requestorsQueues;
}
...
MemoryController::~MemoryController() // dtor
{
	...
	requestorsQueues.clearRequests();
	myTrace.close();
}
...
void MemoryController::connectMemoryDevice(MemoryDevice *memDev)
{
	...
	schedulerRegister = new SchedulerRegister(dataBusWidth, requestorCriticalTable, requestQueue, commandQueue, requestorsQueues);
...
bool MemoryController::addRequest(unsigned int requestorID, unsigned long long address, bool R_W, unsigned int size, unsigned long long reqID)
{
	...
	// allow hitting in requestQueues before being inserted with pendingRequests
	if (R_W) //if read request
	{
		for (unsigned int requestor= 0; requestor < requestQueue[queueIndex]->getQueueSize(); requestor++) // iterate over requestors
		{
			for (unsigned int request = 0; request < requestQueue[queueIndex]->getSize(true,requestor); request++) // iterate over requests
			{
				checkRequest = requestQueue[queueIndex]->checkRequestIndex (requestor,request);
				if ((checkRequest->address == address) && (checkRequest->requestType == DATA_WRITE))
				{
					hitRequest = NULL;
					hitRequest = incomingRequest;
					cout << "MCsim: Request hit in MC: "<< incomingRequest->reqID<<" WR: "<< (unsigned long long)checkRequest->reqID << " type: " << checkRequest->requestType << " " << checkRequest->address << " "<< address << endl;
					return true;
				}
			}
		}
