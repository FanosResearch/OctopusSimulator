...
namespace MCsim
{
																																																					...
																																																					numberRanks(ranks_)
	{
		...
		{
			std::cout << "Wrong DRAM standard "<< deviceGene << std::endl;
		}
...
*/
	...
	bool MemorySystem::addRequest(unsigned int requestorID, unsigned long long address, bool R_W, unsigned int size, unsigned long long reqID)
	{
		if(requestorsQueues == NULL)
		{
			std::cout <<"Memory system null pointer " <<std::endl;
		}
		return memoryController->addRequest(requestorID, address, R_W, size, reqID);
	}
