	...
	public:
		...
		bool commandGenerate(Request *request, bool open)
		{
			...
			{ // Assuming the initial state of the banks in device is idle
				commandBuffer.push(new BusPacket(PRE, id, address, col, row, bank, rank, sa, NULL, 0, request->reqID, dataBusSize));
				commandBuffer.push(new BusPacket(ACT, id, address, col, row, bank, rank, sa, NULL, 0, request->reqID, dataBusSize));
			}
			...
			{
				commandBuffer.push(new BusPacket(CAS, id, address, col + x, row, bank, rank, sa, request->data, 0, request->reqID, dataBusSize));
			}
			return true;
		}
	};
