...
cores=4
# BMs_path="/Users/Mhossam/Documents/PhD_Work/CacheSim_cmake/cachesim/BMs/Splash/4Cores/"
BMs_path="${BMs_ROOT}/Splash/4Cores/"
...
run()
{
    ...
    # for ((i=8; i<=64; i+=8))
    # do
    #     echo "*************Size Limit ${i}*************" >>"${BMs_path}${protocol}_output.txt"
    #     simulate FFT $protocol $i $OoO_requests
    # done

    # for ((i=2; i<=20; i+=2))
    # do
    #     echo "*************OoO Requests ${i}*************" >>"${BMs_path}${protocol}_output.txt"
    #     simulate FFT $protocol $size_limit $i
    # done

    simulate FFT $protocol $size_limit 20
}
...
simulate()
{
    ...
    sim+=" -p 'cpu[*].m_number_of_OoO_requests(i)=${OoO_requests}'"
    ...
    sim+=" -p 'RROF_size_limit(i)=$size_limit'",
