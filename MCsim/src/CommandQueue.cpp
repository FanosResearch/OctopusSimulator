...
void CommandQueue::clearQueue(bool critical)
{
	if (critical)
		hrtBuffer.clear();
	else
...
BusPacket *CommandQueue::checkLastCommand(bool critical) // Check a cmd from general buffers while for the purpose of checking
{
	if (critical && !hrtBuffer.empty())
	{
		return hrtBuffer.back();
	}
	else if (!critical && !srtBuffer.empty())
	{
		return srtBuffer.back();
	}
	else
	{
		DEBUG("COMMAND QUEUE: CHECK THE SIZE OF checkLastCommand " <<getSize (critical));
		abort();
		return NULL;
	}
}
