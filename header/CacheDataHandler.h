...
namespace ns3
{
    ...
    public:
        ...
        virtual void makeSpaceFor(uint64_t address);
        ...
        virtual uint64_t tag_mask() 
        {
            int num_bits = (int)log2(m_block_size) + (int)log2(m_sets_count);
            uint64_t bits_mask = (1 << num_bits) - 1;
            return ~bits_mask;
        };
    };
}
