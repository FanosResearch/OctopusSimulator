...
public:
    ...
    uint16_t owner = 0;
    uint16_t from = 0;
    // Previous hop (interface id of the node that most recently transmitted
    // this message). Used by NoCController::step() to decide link direction
    // at multi-hop forwarding. Separate from `from`, which remains the
    // PROTOCOL-level original sender (the protocol uses msg.from to detect
    // e.g. DataFromOwner vs Data-from-LLC).
    uint16_t prev_hop = 0;
    ...
    {
        return this->msg_id == M2.msg_id;
    }

    bool operator==(int id)
    {
        return this->msg_id == id;
    }
};
