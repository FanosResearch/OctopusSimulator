...
namespace ns3
{
    ...
    protected:
        ...
        Logger();
        virtual void prepareReportFile(uint64_t core_id);
        virtual void initializeStats(uint64_t core_id);
        virtual void calculateLatencies(uint64_t msg_id);
    ...
    public:
        virtual void addRequest(uint64_t cpu_id, Message&);
        virtual void updateRequest(uint64_t msg_id, EntryId entryId);
        virtual void registerReportPath(std::string file_path);
        virtual void traceEnd(uint64_t core_id);
        virtual void setClkCount(uint64_t core_id, uint64_t clk);
