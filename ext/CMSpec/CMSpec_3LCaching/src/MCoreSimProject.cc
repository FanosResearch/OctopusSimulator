...
void MCoreSimProject::setup2(MCoreSimProjectXml projectXmlCfg)
{
  ...
  // m_fsm_protocol_path = path.substr(0, app_name_index + sizeof(APP_NAME) - 1) + string("/Protocols_FSM/");
  // m_fsm_llc_protocol_path = path.substr(0, app_name_index + sizeof(APP_NAME) - 1) + string("/Protocols_FSM/");
  m_fsm_protocol_path = "/home/gem5/gem5_new/ext/CMSpec/CMSpec/Protocols_FSM/";
  m_fsm_llc_protocol_path = "/home/gem5/gem5_new/ext/CMSpec/CMSpec/Protocols_FSM/";
  cout <<"FSM path: " << m_fsm_protocol_path << endl;
...
void MCoreSimProject::setup3(MCoreSimProjectXml projectXmlCfg)
{
  ...
  m_fsm_llc_protocol_path = "/home/gem5/gem5_new/ext/CMSpec/CMSpec/Protocols_FSM//MSI_LLC.csv";
  cout <<"FSM path: " << m_fsm_protocol_path << endl;
  ...
  {    
    DirectInterconnect *cpu_interconnect = new DirectInterconnect(-1, L1->GetCacheId(), projectXmlCfg.GetCpuFIFOSize());
    
    stringstream bmTraceFile, cpuTraceFile, ctrlTraceFile;
    bmTraceFile << projectXmlCfg.GetBMsPath() << "/trace_C" << L1->GetCacheId() << ".trc.shared";

    // new CPU(*L1, projectXmlCfg.GetOutOfOrderStages(), cpu_interconnect->getInterfaceFor(-1), bmTraceFile.str());

    ExternalCPU* external_cpu = new ExternalCPU(*L1, cpu_interconnect->getInterfaceFor(-1));
    external_cpu->registerInitializableMemory(m_SharedCacheCtrl);
    ExternalCPU::getExtCPUs()->emplace(L1->GetCacheId(), external_cpu);
    ...
    CacheController*  L1_cache = new CacheController(*L1, l1_fsm, l1_interconnect->getInterfaceFor(L1->GetCacheId()), 
                                                    cpu_interconnect->getInterfaceFor(L1->GetCacheId()),
    ...
    L2_cache = new CacheController(*L2, l2_fsm, bus_interface, 
                                   ...
                                   projectXmlCfg.GetCache2Cache(), xmlSharedCache.GetCacheId(), CohProtType::L2_MSI);

    m_cpuCacheCtrl.push_back(L2_cache);
    L2_cache->m_children_controllers.push_back(L1_cache);
    m_SharedCacheCtrl->m_children_controllers.push_back(L2_cache);
    cpu_interconnect->init();
