...
namespace ns3
{
    ...
    ExternalMem::ExternalMem(MCoreSimProjectXml &xml, CommunicationInterface *lower_interface, vector<int> llc_id) : ClockedObj(xml.GetDRAMCtrlClkNanoSec())
    {
        m_id = xml.GetDRAMId()[0];
        m_llc_id = llc_id;
        ...
        m_llc_line_size = xml.GetSharedCaches().begin()->GetBlockSize();
