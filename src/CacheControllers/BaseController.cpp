...
namespace ns3
{
    ...
    : ClockedObj(0), Configurable(map, config_path, name, pname)
    {
        ...
        // Was `reserve(...)` which only allocates capacity; operator[] writes
        // below were past-the-end (UB). Older compilers happened not to crash;
        // GCC 13 + ASan reliably faults during destruction of the corrupted
        // std::function. resize() makes the slots actually exist.
    ...
    void BaseController::addRequests2ProcessingQueue(ProcessingBuffer<Message> &buf)
    {
        Message msg;
