...
namespace ns3
{
    ...
    {   
        ...
        m_data_access_arbiter = createArbiter(arbiter_type, arbiter_candidates_ids, m_data_handler->getDataAccessLatency());
    ...
    CacheController::~CacheController()
    {
        delete m_protocol;
        delete m_data_handler;
        if(m_data_access_arbiter)
            delete m_data_access_arbiter;
    }

    Arbiter* CacheController::createArbiter(string arbiter_type, vector<int>* candidates_ids, int arbiter_period)
    {
        if(arbiter_type == STRINGIFY(NULL))
            return NULL;
    ...
    {
        if(m_data_handler->isReady() && m_data_access_arbiter != NULL && !m_data_access_buffer.empty())
        {
    ...
    void CacheController::addRequests2ProcessingQueue(ProcessingBuffer<Message> &buf)
    {
    ...
    void CacheController::hitAction(void *data_ptr)
    {
        ...
        if(!getDataFromDataAccessBuf(msg)) //check if a data write is pending in m_data_access_buffer to get data from
        {
            if (msg->data == NULL && 
    ...
    void CacheController::performWriteBack(void *data_ptr)
    {
        ...
        if(!getDataFromDataAccessBuf(msg)) //check if a data write is pending in m_data_access_buffer to get data from
        {
            ...
            {
                cout << "LLC: not ready data array for WB\n";
                return;
            }
        }
    ...
    void CacheController::stall(void *data_ptr)
    {
        ...
        if (!m_processing_queue->pushBack(*msg, EntryState::NonReady))
        {
    ...
    bool CacheController::getDataFromDataAccessBuf(Message *msg)
    {
        auto itr = find_if(m_data_access_buffer.begin(), m_data_access_buffer.end(), 
                            [&] (Message buf_msg) -> bool{
                                return buf_msg.data != NULL &&
                                       this->getAddressKey(msg->addr) == this->getAddressKey(buf_msg.addr);
                            });
        if(itr != m_data_access_buffer.end())
        {
            msg->copy(itr->data);
            return true;
        }
    ...
    void CacheController::checkReplacements(ProcessingBuffer<Message> &buf)
    {
