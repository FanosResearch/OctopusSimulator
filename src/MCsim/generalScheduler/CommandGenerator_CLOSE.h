		...
		bool commandGenerate(Request *request, bool open)
		{
			...
			for (unsigned int bi = 0; bi < lookupTable[request->requestSize].first; bi++)
			{
				...
				for (unsigned int bc = 0; bc < lookupTable[request->requestSize].second; bc++)
				{
					...
					commandBuffer.push(new BusPacket(ACT, id, address, col, row, bank, rank, sa, 0, 0, request->reqID, dataBusSize));
					commandBuffer.push(new BusPacket(CAS, id, address, col, row, bank, rank, sa, request->data, 0, request->reqID, dataBusSize));
				}
			}
			return true;
		}
