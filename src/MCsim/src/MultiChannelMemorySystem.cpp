	  ...
	  clockDomainCrosser(new ClockDomain::Callback<MultiChannelMemorySystem, void>(this, &MultiChannelMemorySystem::actual_update))
{
	...
	for (size_t i = 0; i < numberChannels; i++)
	{
		...
		requestorsQueues = channel->requestorsQueues;
...
bool MultiChannelMemorySystem::addRequest(unsigned int requestorID, unsigned long long address, bool R_W, unsigned int size, unsigned long long reqID)
{
	unsigned channelNumber = findChannelNumber(requestorID, address);
	//requestorsQueues->addRequest(requestorID, reqID,1);
	return channels[channelNumber]->addRequest(requestorID, address, R_W, size, reqID);
}
...
RequestorsQueues * MultiChannelMemorySystem::getRequestorsQueues()
{
	return requestorsQueues;
}
namespace MCsim
{
