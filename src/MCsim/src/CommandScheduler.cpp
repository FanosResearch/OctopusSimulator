			...
			if (isIssuableRefresh(tempCmd))
			{
				if (tempCmd->busPacketType == PRE)
				{
					TRACE_CMD("TRACE-COMMAND:PRE"
							  << "\t" << clock << ":"
							  << "\t\tAddress: " << tempCmd->address << "\t\tBank: " << tempCmd->bank << "\t\tColumn: " << tempCmd->column << "\tRow: " << tempCmd->row << "\tReqID: " << tempCmd->reqID);
				}
	...
	if (commandQueue[index]->isPerRequestor())
	{
		...
		if (cmd->busPacketType == PRE)
		{
			if (cmd->address > 999999)
			{
				TRACE_CMD("TRACE-COMMAND:PRE"
						  << "\t" << clock << ":"
						  << "\t\tAddress: " << cmd->address << "\tBank: " << cmd->bank << "\t\tColumn: " << cmd->column << "\tRow: " << cmd->row << "\tReqID: " << cmd->reqID);
			}
			else
			{
				TRACE_CMD("TRACE-COMMAND:PRE"
