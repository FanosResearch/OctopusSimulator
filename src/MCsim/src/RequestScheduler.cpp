...
bool RequestScheduler::isRowHit(Request *request)
{
	...
	{
		if (bankTable[request->addressMap[Rank]].find(request->addressMap[Bank]) != bankTable[request->addressMap[Rank]].end())
		{
	...
	if (request->requestType == DATA_READ)
	{
		if (request->address > 999999)
		{
			TRACE_REQ("TRACE-REQUEST:READ"
					  << "\t\t" << clockCycle << ":"
					  << "\t\tAddress: " << request->address << "\tBank: " << request->bank << "\t\tColumn: " << request->col << "\t\tRow: " << request->row << " \t\tCore: "<< request->requestorID << "\t\tReqID: " <<request->reqID);
		}
		else
		{
			TRACE_REQ("TRACE-REQUEST:READ"
