...
AddressMapping::AddressMapping(const std::string &format, const unsigned int (&memTable)[6])
{
	...
	for (unsigned index = 0; index < format.size(); index++)
	{
		...
		if (decodeIndex[index].first == memOrg::Column)
			decodeIndex[index].second -= 3;
	}
}
...
{
		int TX_SIZE_BITS=6;
		long long int temp_address = request->address >> TX_SIZE_BITS;
	...
	unsigned tempA = temp_address;
	unsigned tempB = temp_address;
	unsigned tempAddr = 0;
...
// Case 2: Single Rank Bank Privatized Controllers
	...
	cout << "SINGLE_RANK_BANK_PRIVATIZATION: req_ID: "<< request->requestorID << endl;
#endif
...
// bank privatization for 2 cores system 
#ifdef CORE2_BANK_PRIVATIZATION
	request->bankGroup = 0;
	request->bank = (request->requestorID <<3) +lowestBitsValue(request->bank,3);
#endif

// bank privatization for 4 cores system 
#ifdef CORE4_BANK_PRIVATIZATION
	request->bankGroup = 0;
	request->bank = (request->requestorID <<2) +lowestBitsValue(request->bank,2);
#endif

// bank privatization for 8 cores system 
#ifdef CORE8_BANK_PRIVATIZATION
	request->bankGroup = 0;
	request->bank = (request->requestorID <<1) +lowestBitsValue(request->bank,1);
#endif

// bank privatization for 16 cores system 
#ifdef CORE16_BANK_PRIVATIZATION
	request->bankGroup = 0;
	request->bank = request->requestorID;
...
// bank privatization for 16 cores system 
#ifdef CORE32_BANK_PRIVATIZATION
	request->bankGroup = 0;
	request->bank = request->requestorID;
...
unsigned int AddressMapping::lowestBitsValue(unsigned int bank, unsigned int bits)
{
	unsigned mask = 1;
	unsigned value;
	for(unsigned x = 0; x < bits-1 ; x++)
	{
		mask = (mask << 1)+1; 
	}
	value = mask & bank;
	//std::cout << " mask: "<<mask << " bank "<< bank << " bits "<< bits << " value "<< value;
	return value;
}
