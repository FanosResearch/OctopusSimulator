...
#include <cstdio>      // snprintf for width-limited sscanf fmt strings
...
{   
    ...
    vector<T> Configurable::parseVector(const char* line)
    {
        ...
        do
        {
            ...
            else if constexpr(is_same<T, string>::value)
            {
                ...
                // Width specifier prevents overflow if the field exceeds
                // MAX_STRING_SIZE-1 chars (e.g. long absolute paths).
                // Original: sscanf(..., ",%[^,\n\r]", &s_value);
                char fmt[32];
                std::snprintf(fmt, sizeof(fmt), ",%%%d[^,\n\r]", MAX_STRING_SIZE - 1);
                ret = sscanf(sline.substr(position).c_str(), fmt, s_value);
    ...
    Parameter Configurable::parseLine(const char* line)
    {
        ...
        // Width-limited %[] prevents overflow on long parameter names.
        // Original: sscanf(line, "%[^(]", param_name);
        char name_fmt[16], ind_fmt[24];
        std::snprintf(name_fmt, sizeof(name_fmt), "%%%d[^(]", MAX_NAME_SIZE - 1);
        std::snprintf(ind_fmt,  sizeof(ind_fmt),  "%%*[^(](%%%d[^)]", MAX_TYPE_SIZE - 1);
        sscanf(line, name_fmt, param_name); //reads from the beginning to '('
        sscanf(line, ind_fmt,  param_indicator); //ignores from the beginning to '(' then reads from '(' to ')'
        ...
        {
            char s_value[MAX_STRING_SIZE] = {0};
            // Width-limited %[] prevents stack-buffer overflow when the
            // string field exceeds MAX_STRING_SIZE-1 (e.g. long paths).
            // Original: sscanf(line, "%*[^,],%[^,\r\n]", s_value);
            char fmt[40];
            std::snprintf(fmt, sizeof(fmt), "%%*[^,],%%%d[^,\r\n]", MAX_STRING_SIZE - 1);
            sscanf(line, fmt, s_value);
