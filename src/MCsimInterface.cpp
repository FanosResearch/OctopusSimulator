...
namespace ns3
{
    MCsimInterface::MCsimInterface(MCoreSimProjectXml &projectXml, CommunicationInterface *lower_interface, int llc_id)
    {
        ...
        m_processing_queue = new ProcessingBuffer<Message>([&] (const Message& msg, EntryState state) -> EntryState {
                                                            return this->getRequestState(msg, state);});
    ...
    void MCsimInterface::processLogic()
    {
        ...
        Message ready_msg;
        if (m_processing_queue->getReady(&ready_msg))
        {
    ...
    void MCsimInterface::addRequests2ProcessingQueue(ProcessingBuffer<Message> &buf)
    {
        Message msg;
        ...
        if (m_lower_interface->peekMessage(&msg))
        {
            ...
            if (buf.pushBack(msg, EntryState::Ready))
                m_lower_interface->popFrontMessage();
        }
    }

    EntryState MCsimInterface::getRequestState(const Message &msg, EntryState current_state)
    {
