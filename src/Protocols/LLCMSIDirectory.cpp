...
namespace ns3
{
    ...
    EntryState LLCMSIDirectory::getRequestState(const Message &msg, EntryState req_state)
    {
        ...
        this->readEvent(copy_msg, cache_line, &event_id);
    ...
    vector<ControllerAction> LLCMSIDirectory::processRequest(Message &request_msg, DebugPrint* dprint)
    {
        ...
        bool caused_replacement = false;
        if(!cache_line.valid && event_id != EventId::Replacement && next_state != 0)
            this->allocateLine(request_msg.addr, &caused_replacement);

        if(!caused_replacement)
            return handleAction(actions, request_msg, cache_line, next_state);
        else
        {
            vector<ControllerAction> controller_actions = handleAction(actions, request_msg, cache_line, next_state);
            ControllerAction controller_action;
            controller_action.type = ControllerAction::Type::CAUSED_REPLACEMENT;
            controller_action.data = (void *)new Message(request_msg);
                                                           ...
                                                           GenericCacheLine &cache_line, int next_state)
    {
        ...
        for (int action : actions)
        {
            ...
            case ActionId::SendInv:
                ...
                break;
            case ActionId::ReSendInv:
                // send Bus request, update cache line
                controller_action.type = ControllerAction::Type::SEND_FWD_MESSAGE;
                controller_action.data = (void *)new Message(msg);

                ((Message *)controller_action.data)->complementary_value = (uint16_t)MSIDirectory::REQUEST_TYPE_INV;
                ((Message *)controller_action.data)->owner = m_id;
                ((Message *)controller_action.data)->to.clear();
                for(uint64_t id : protocol_counters->at(msg.addr & ~uint64_t(m_data_handler->getBlockSize() -1)))
                    ((Message *)controller_action.data)->to.push_back((uint16_t)id);
            ...
            case ActionId::WriteBack:
                ...
                controller_action.data = (void *)new Message(msg.msg_id, // Id //should add data
                                                             msg.addr,   // Addr
                                                             0,          // Cycle
        ...
        if(actions[0] != (int)ActionId::Stall)
        {
            // update cache line
            ControllerAction controller_action;
            Message new_msg = msg;
            ...
            if(new_msg.data != NULL && save_data == false)
                new_msg.clearData();
            ...
            new (controller_action.data) Message(new_msg);
        ...
        case Message::Source::LOWER_INTERCONNECT:
            ...
            else
            {
                ...
                case MSIDirectory::REQUEST_TYPE_PUTS:
                {
                    ...
                    {
                        if(protocol_counters->at(addr_key).front() == msg.owner) //ensure that the message from the last sharer
                            *out_id = EventId::last_PutS;
                        else
                            *out_id = EventId::PutS;
                    }
                    else if((protocol_counters->find(addr_key) == protocol_counters->end()))
                        *out_id = EventId::last_PutS;
                    else
                        *out_id = EventId::PutS;
