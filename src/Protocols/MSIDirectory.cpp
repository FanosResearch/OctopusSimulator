...
namespace ns3
{
    ...
    EntryState MSIDirectory::getRequestState(const Message &msg, EntryState req_state)
    {
    ...
    vector<ControllerAction> MSIDirectory::processRequest(Message &request_msg, DebugPrint* dprint)
    {
        ...
        // Debug: emit transition info on Fault transitions so we can see
        // which (state, event) combination is unexpected.
        for(int a : actions) {
            if(a == (int)ActionId::Fault) {
                std::cout << "  [Fault trigger] state=" << (int)cache_line.state
                          << " event=" << (int)event_id
                          << " addr=0x" << std::hex << request_msg.addr << std::dec
                          << " from=" << request_msg.from
                          << " owner=" << request_msg.owner
                          << std::endl;
                break;
            }
        }
                                                        ...
                                                        GenericCacheLine &cache_line, int next_state)
    {
        ...
        for (int action : actions)
        {
            ...
            case ActionId::Fault:
                ...
                std::cout << "   addr=0x" << std::hex << msg.addr << std::dec
                          << " msg_id=" << msg.msg_id
                          << " owner=" << msg.owner
                          << " from=" << msg.from
                          << " state=" << (int)cache_line.state
