...
namespace ns3
{
                                                           ...
                                                           GenericCacheLine &cache_line, int next_state)
    {
        ...
        int line_owner = cache_line.owner_id;
        ...
        if(flags[ActionId::SendInv]) //override sendInv of MSI
        {
            ...
            for(uint64_t id : protocol_counters->at(msg.addr & ~uint64_t(m_data_handler->getBlockSize() -1)))
            {
                if(id == msg.owner) //skip invalidation for the message owner
                    continue;
                if(id == line_owner && msg.owner != m_id) //skip invalidation of a previous owner as it will be invalidated by a fwd message
                    continue;
        ...
        if(flags[ActionId::FwdGetM]) //override fwdGetM of MSI
        {
            ...
            ((Message *)controller_action.data)->to.clear();
            ((Message *)controller_action.data)->to.push_back(line_owner);
            ((Message *)controller_action.data)->complementary_value2 = 0;
            for(uint64_t id : protocol_counters->at(msg.addr & ~uint64_t(m_data_handler->getBlockSize() -1)))
            {
                if(id == msg.owner) //skip invalidation for the message owner
                    continue;
                if(id == line_owner) //skip invalidation of a previous owner as it will be invalidated by a fwd message
                    continue;
        ...
        if(flags[ActionId::SendInvM])
        {
            ...
            ((Message *)controller_action.data)->to.clear();
            ((Message *)controller_action.data)->to.push_back((uint16_t)line_owner);
