...
namespace ns3
{
                                   ...
                                   string pname, string config_path, string name) : Configurable(pmap, config_path, name, pname)
    {
        ...
        m_candidates_ids = std::get<vector<int>>(parameters.at(STRINGIFY(m_candidates_ids)).value);

        //Constructor
        if(m_candidates_ids.empty())
        {
            for(auto interface : *interfaces)
                m_candidates_ids.push_back(interface->m_interface_id);
        }
    ...
    void MeshController::initializeLinks(map<int, vector<int>> *topology)
    {
        ...
        for(auto& link : links)
        {
            ...
            {
                Arbiter* arbiter = new FCFSArbiter(&link.ids, link.latency);
                ((FCFSArbiter*)arbiter)->setRespectOrder();
                link.arbiter = arbiter;
            }
            ...
            {
                Arbiter* arbiter = new RROFArbiter(&m_candidates_ids, link.latency);
                ((RROFArbiter*)arbiter)->set_arbiter_name("interconnect", -1);
                link.arbiter = arbiter;
            }
            else
            {
