...
namespace ns3
{
        ...
        : BusController(interfaces, lower_level_ids, memArb)
    {
        ...
        if(candidates_id == NULL)
		    candidates = m_lower_level_ids;
        else
		    candidates = candidates_id;

	        if (memArb == "RR")
                m_arbiters.push_back(new RRArbiter(candidates, m_response_latency + m_request_latency));
            else if (memArb == "RROF")
                m_arbiters.push_back(new RROFArbiter(candidates, m_response_latency + m_request_latency));
            else if (memArb == "TDM")
                m_arbiters.push_back(new TDMArbiter(candidates, m_response_latency + m_request_latency));
            else
                m_arbiters.push_back(new FCFSArbiter(candidates, m_response_latency + m_request_latency));
