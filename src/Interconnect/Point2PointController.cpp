...
namespace ns3
{
            ...
            string pname, string config_path, string name) : BusController(map, interfaces, lower_level_ids, pname, config_path, name)
    {
        ...
        {
            Arbiter* arbiter = new RROFArbiter(ids_ptr, m_request_latency + m_response_latency);
            ((RROFArbiter*)arbiter)->set_respect_order();
        ...
        // Upper counterparts were left uninitialized; first read at line 135
        // got a garbage byte (UBSan flagged value=190) which kept the upper
        // bus in a stuck state until the garbage happened to land on false.
        clk_in_slot_upper = 0;
        ...
        if(arbiter_type == STRINGIFY(RROFArbiter))
            ((RROFArbiter*)m_arbiters[0])->set_arbiter_name("sBus", upper_interface[0]->m_interface_id);
    }
