...
namespace ns3
{
    ...
    void CPU::processLogic()
    {
        ...
        if (m_sent_requests < m_number_of_OoO_requests)
        {
            ...
            {
                if(RROFArbiter::spaceForRequest(m_id) && m_upper_interface->pushMessage(m_sample_in_progess->msg))
                {
                    // if(m_id == 0 && m_sample_in_progess->msg.msg_id >= 191)
                    //     cout << "here " << m_sample_in_progess->msg.msg_id << endl;
                    Logger::getLogger()->addRequest(this->m_id, m_sample_in_progess->msg);
                    RROFArbiter::addDemandReq2Queues(m_sample_in_progess->msg);
                    delete m_sample_in_progess;
                    m_sample_in_progess = NULL;
                    m_sent_requests++;
                }
